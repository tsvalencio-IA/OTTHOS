...
async function head(p){try{const r=await fetch(p+'?v545='+Date.now(),{method:'HEAD',cache:'no-store'});add('Arquivo '+p,r.ok,'HTTP '+r.status)}catch(e){add('Arquivo '+p,false,e.message)}}
...
console.log('%cATHOS V54.5 CLAREZA INIMIGOS/OBSTÁCULOS','font-size:20px;color:#55ff55;background:#111;padding:8px');
for(const f of ['./index.html','./style.css','./app.js','./sw.js','./assets/render-v54/v54-render-premium.js','./assets/render-v54/v54-render-premium.css']) await head(f);
add('Título V54.5',/V54\.5/i.test(document.title),document.title);
await tap($('#playBtn')||$('#heroPlayBtn'),'Jogar');
await sleep(4300);
const api=window.ATHOS_TEST_API||{};
add('V54.5 ativo',!!api.getV545?.(),JSON.stringify(api.getV545?.()||{}));
add('Render sem falsos interativos',api.getV54Render?.()?.fakeInteractiveRemoved===true,JSON.stringify(api.getV54Render?.()||{}));
