...
async function head(p){try{const r=await fetch(p+'?v541='+Date.now(),{method:'HEAD',cache:'no-store'});add('Arquivo '+p,r.ok,'HTTP '+r.status)}catch(e){add('Arquivo '+p,false,e.message)}}
...
console.log('%cATHOS V54.1 CODEX RENDER MOBILE INTERATIVO','font-size:20px;color:#55ff55;background:#111;padding:8px');
...
add('Título V54.1',/V54\.1/i.test(document.title),document.title);
...
add('V54.1 ativo',!!api.getV541?.(),JSON.stringify(api.getV541?.()||{}));
add('Render V54 carregado',!!api.getV54Render?.()?.installed,JSON.stringify(api.getV54Render?.()||{}));
const joy=$('#joystick'), grid=$('.action-grid'), hud=$('.top-hud'), obj=$('.objective-card'), settings=$('#hudSettingsBtn'), plus=$('#hudGemPlusBtn')||$('.hud-gem-plus'), worlds=$('.world-strip');
add('Mundos fora da tela de jogo',!visible(worlds),'mundos agora em Configurações');
...
add('Botões com espaço',visible(grid)&&!Array.from(grid.children).some((b,i,a)=>visible(b)&&a.some((c,j)=>j>i&&visible(c)&&overlap(b,c))),'sem sobreposição entre botões');
...
add('Config tem ações extras',all('[data-settings-action]').length>=5,`${all('[data-settings-action]').length} ações`);
$('#modalClose')?.click();
await sleep(250);
...
await tap($('.settings-world-btn[data-settings-world="real"]')||$('.world-chip[data-world="real"]')||$('#arNativeExternalBtn'),'Real/AR');
await sleep(700);
