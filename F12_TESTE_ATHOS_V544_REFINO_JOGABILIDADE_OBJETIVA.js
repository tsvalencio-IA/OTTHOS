...
async function head(p){try{const r=await fetch(p+'?v544='+Date.now(),{method:'HEAD',cache:'no-store'});add('Arquivo '+p,r.ok,'HTTP '+r.status)}catch(e){add('Arquivo '+p,false,e.message)}}
...
console.log('%cATHOS V54.4 REFINO JOGABILIDADE OBJETIVA','font-size:20px;color:#55ff55;background:#111;padding:8px');
...
add('Título V54.4',/V54\.4/i.test(document.title),document.title);
...
add('V54.4 ativo',!!api.getV544?.(),JSON.stringify(api.getV544?.()||{}));
...
add('Objetivo não genérico vazio',/espada|Cristais|Monstros|Portal|Ação/i.test($('#objectiveText')?.textContent||''),$('#objectiveText')?.textContent||'');
add('Mundos removidos do gameplay',!$('.game.active .game-controls > .world-strip'),'sem world-strip');
...
await tap($('#hudSettingsBtn'),'Configurações');
await sleep(250);
add('Configurações abre',visible($('#modal')) && all('.settings-world-btn').length>=7,`${all('.settings-world-btn').length} mundos`);
$('#modalClose')?.click();
await sleep(250);
let p0=api.getPlayerState?.(); await sleep(450); let p1=api.getPlayerState?.();
add('Athos parado não anda sozinho',p0&&p1&&Math.abs((p1.x-p0.x)||0)<.08&&Math.abs((p1.z-p0.z)||0)<.08&&p1.y>=0,JSON.stringify({antes:p0,depois:p1}));
api.power?.();
await sleep(450);
add('B Poder sem erro',errors.length===0,JSON.stringify(api.getGameplayState?.()||{}));
