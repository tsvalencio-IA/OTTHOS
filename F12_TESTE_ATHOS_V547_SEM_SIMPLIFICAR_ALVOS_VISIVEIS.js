...
async function head(p){try{const r=await fetch(p+'?v547='+Date.now(),{method:'HEAD',cache:'no-store'});add('Arquivo '+p,r.ok,'HTTP '+r.status)}catch(e){add('Arquivo '+p,false,e.message)}}
...
console.log('%cATHOS V54.7 SEM SIMPLIFICAR + ALVOS VISÍVEIS','font-size:20px;color:#55ff55;background:#111;padding:8px');
...
add('Título V54.7',/V54\.7/i.test(document.title),document.title);
await tap($('#playBtn')||$('#heroPlayBtn'),'Jogar');
await sleep(4300);
const api=window.ATHOS_TEST_API||{};
add('V54.7 ativo',!!api.getV547?.(),JSON.stringify(api.getV547?.()||{}));
add('Render rico preservado',api.getV54Render?.()?.renderRichPreserved===true,JSON.stringify(api.getV54Render?.()||{}));
add('Sem falsos interativos',api.getV54Render?.()?.fakeInteractiveRemoved===true,JSON.stringify(api.getV54Render?.()||{}));
