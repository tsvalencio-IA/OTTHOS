...
async function head(p){try{const r=await fetch(p+'?v533='+Date.now(),{method:'HEAD',cache:'no-store'});add('Arquivo '+p,r.ok,'HTTP '+r.status)}catch(e){add('Arquivo '+p,false,e.message)}}
...
console.log('%cATHOS V53.3 CONTROLES DENTRO DA TELA','font-size:20px;color:#55ff55;background:#111;padding:8px');
...
add('Título V53.3',/V53\.3/i.test(document.title),document.title);
...
add('V53.3 ativo',!!api.getV533?.(),JSON.stringify(api.getV533?.()||{}));
