...
(async () => {
  ...
  try {
    ...
    await safe('Inicialização', 'API pública de testes V606 disponível', 'critical', () => ({
      passed: !!api && api.version === 'V606_STABLE_ROLEPLAY_FULL',
    ...
    await safe('Personagem', 'Otthos procedural fiel é usado na jogabilidade', 'critical', () => {
      const visual = api.getVisual?.() || {};
      return {
        passed: visual.procedural === true && visual.visualVersion === 'V606_FIEL_REFERENCIA',
        details: `procedural=${visual.procedural}; visual=${visual.visualVersion || 'ausente'}`,
        metrics: visual
      };
    });

    await safe('Personagem', 'Base visual dos pés permanece alinhada ao solo', 'critical', () => {
      const visual = api.getVisual?.() || {};
      const game = api.getGame?.() || {};
      const expected = Number(game.player?.y || 0);
      const delta = Math.abs(Number(visual.visualBottom || 0) - expected);
      return {
        passed: Number.isFinite(delta) && delta <= 0.10,
        details: `base visual=${round(visual.visualBottom,3)}, solo físico=${round(expected,3)}, diferença=${round(delta,3)}`,
        metrics: { visualBottom: visual.visualBottom, physicalY: expected, delta }
      };
    });
    ...
    await safe('Casa', 'Entrada muda para câmera interna', 'critical', async () => {
      await api.exitHouse?.();
      const entered = await api.enterHouseById('home');
      await wait(420);
      const game = api.getGame();
    ...
    await safe('Mapa', 'Mapa GPS abre com posição, direção e destinos clicáveis', 'critical', async () => {
      const mapButton = qs('#mapBtn');
      mapButton?.click();
      await wait(180);
      const pins = qsa('.map-pin');
      const player = qs('.player-dot');
      const compass = qs('.player-dot i');
      const routeList = qs('.map-scroll');
      const passed = pins.length >= 9 && !!player && !!compass && !!routeList && visible(qs('#modal'));
      closeModal();
      return {
        passed,
        details: `pins=${pins.length}, jogador=${!!player}, direção=${!!compass}, lista=${!!routeList}`
      };
    });

    await safe('Navegação', 'Guia no mundo existe e aceita destino real', 'high', async () => {
      const map = api.map();
      const target = map.locations?.find(item => item.id === 'gym') || map.locations?.[0];
      if (!target) return { passed:false, details:'Nenhum destino disponível' };
      const state = api.getState();
      const hasGuide = !!qs('#navGuide') && !!qs('#navArrow') && !!qs('#navDistance');
      return {
        passed: hasGuide && Number.isFinite(target.x) && Number.isFinite(target.z),
        details: `guia=${hasGuide}, destino=${target.name}, coordenadas=(${target.x},${target.z})`
      };
    });
    ...
    await safe('Salvamento', 'IndexedDB possui schema e save válido', 'critical', async () => {
      ...
      return {
        passed: status.schema === 606 && !!loaded?.profile?.playerId,
    ...
    await safe('Salvamento', 'Cópia local de emergência é válida', 'high', () => {
      ...
      return {
        passed: !!parsed?.profile?.playerId && parsed.version === 606,
    ...
    try {
      ...
      if (CONFIG.restoreAfterTest && api && original) {
        ...
        if (original.currentHouse) {
          await api.enterHouseById?.(original.currentHouse);
        }
