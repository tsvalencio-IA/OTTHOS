(async()=>{
  ...
  if(!api){console.error('OTTHOS_TEST_API não encontrada. Abra a V609 publicada e execute novamente.');return;}

  const original=clone(api.getGame().player),originalState=clone(api.getState()),originalWasVehicle=!!original.vehicle;
  try{
    add('Versão V609 carregada',api.version==='V609_VEHICLE_INPUT_STABLE',api.version);
    ...
    add('IndexedDB schema V609',db?.schema===609,JSON.stringify(db));
    ...
    add('Service Worker do escopo atual é V609',regs.length===0||urls.some(u=>u.includes('v=609')),urls.join(' | ')||'Não disponível');
    ...
    try{
      ...
      localStorage.setItem('otthos_life_world_roleplay_v609',JSON.stringify({...originalState,version:609}));
      if(window.OTTHOS_DB)await window.OTTHOS_DB.save({...originalState,version:609}).catch(()=>false);
  ...
  console.group(`OTTHOS V609 — TESTE VEÍCULO/ENTRADAS ${score}%`);console.table(results);console.log({score,aprovados:ok,total:results.length,results});console.groupEnd();
  document.getElementById('v609-test-panel')?.remove();
  const panel=document.createElement('div');panel.id='v609-test-panel';panel.style.cssText='position:fixed;inset:12px;z-index:2147483647;background:#071522f5;color:white;border:2px solid #5de1ff;border-radius:18px;padding:16px;overflow:auto;font:14px system-ui';
  panel.innerHTML=`<button style="float:right;padding:10px" onclick="this.parentElement.remove()">Fechar</button><h2>Teste V609 — ${score}%</h2><p>${ok} de ${results.length} aprovados.</p><p><b>Nota:</b> a física é validada por passos fixos, sem depender do requestAnimationFrame, que pode ser reduzido enquanto o foco está no DevTools.</p><table style="width:100%;border-collapse:collapse">${results.map(x=>`<tr style="background:${x.Status==='OK'?'#164c2c':'#692132'}"><td style="padding:8px;border-bottom:1px solid #ffffff22">${x.Teste}</td><td style="padding:8px"><b>${x.Status}</b></td><td style="padding:8px">${x.Detalhes}</td></tr>`).join('')}</table>`;
