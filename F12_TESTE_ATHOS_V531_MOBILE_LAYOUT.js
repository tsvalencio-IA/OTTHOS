...
async function head(p){try{const r=await fetch(p+'?v531='+Date.now(),{method:'HEAD',cache:'no-store'});add('Arquivo '+p,r.ok,'HTTP '+r.status)}catch(e){add('Arquivo '+p,false,e.message)}}
...
console.log('%cATHOS V53.1 MOBILE LAYOUT','font-size:20px;color:#55ff55;background:#111;padding:8px');
...
add('Título V53.1',/V53\.1|MOBILE/i.test(document.title),document.title);
