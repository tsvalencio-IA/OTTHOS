...
(async () => {
  ...
  const VERSION = 'V604_COMMERCIAL_GAMEPLAY_AUDIT_1';
  ...
  function buildPanel(report) {
    ...
    panel.innerHTML = `
      ...
      <header>
        <div>
          <h2>Auditoria de Jogabilidade Comercial — Otthos V604</h2>
    ...
    qs('[data-json]', panel).onclick = () => {
      ...
      anchor.href = url;
      anchor.download = `otthos-v604-auditoria-comercial-${new Date().toISOString().replace(/[:.]/g, '-')}.json`;
      anchor.click();
  ...
  function reportToText(report) {
    const lines = [
      `OTTHOS V604 — AUDITORIA DE JOGABILIDADE COMERCIAL`,
  ...
  try {
    ...
    await safe('Inicialização', 'API pública de testes V602 disponível', 'critical', () => ({
      passed: !!api && api.version === 'V604_ROLEPLAY_PROCEDURAL_OTTHOS_COMPLETE',
      details: api?.version || 'OTTHOS_TEST_API ausente'
    }));

    if (!api) throw new Error('OTTHOS_TEST_API não foi encontrada. Abra a V602 e execute novamente.');
    ...
    await safe('Física', 'Otthos procedural fiel mantém os pés ancorados no ponto físico', 'critical', async () => {
      await wait(180);
      const game = api.getGame();
      const visual = game.visual || {};
      const delta = Number(visual.footDelta);
      return {
        passed: visual.faithful === true && visual.source === 'procedural' && Number.isFinite(visual.baseY) && Number.isFinite(visual.modelY) && visual.modelY >= -0.001 && Math.abs(delta) <= 0.12,
        details: `Fiel=${!!visual.faithful}, fonte=${visual.source}, baseY=${round(visual.baseY,3)}, modelY=${round(visual.modelY,3)}, oscilação=${round(delta,3)}`,
        metrics: visual
      };
    });
    ...
    await safe('Mapa', 'GPS possui posição, direção, limites e destino navegável', 'critical', () => {
      const map = api.map();
      const passed = Number.isFinite(map?.player?.facing) && map?.bounds?.minX === -116 && map?.bounds?.maxX === 116 && map?.nearest?.name && map.locations.length >= 14;
      return {
        passed,
        details: `Perto de ${map?.nearest?.name || 'desconhecido'}; direção=${round(map?.player?.facing,3)}; destinos=${map?.locations?.length || 0}`,
        metrics: map
      };
    });
    ...
    await safe('Salvamento', 'IndexedDB possui schema e save válido', 'critical', async () => {
      ...
      return {
        passed: status.schema === 602 && !!loaded?.profile?.playerId,
    ...
    await safe('Salvamento', 'Cópia local de emergência é válida', 'high', () => {
      ...
      return {
        passed: !!parsed?.profile?.playerId && parsed.version === 602,
  ...
  console.group(`%cOTTHOS V604 — AUDITORIA COMERCIAL ${score}/100`, 'font-size:16px;font-weight:bold;color:#52d8ff');
  console.log(verdict);
