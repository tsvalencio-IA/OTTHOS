(() => {
  ...
  let platforms = [], hazards = [], crystals = [], enemies = [], fireballs = [], particles = [], solids = [], gates = [], checkpoints = [], premiumVisuals = [], powerups = [];
  ...
  function clearLevel(){
    ...
    platforms=[]; hazards=[]; crystals=[]; enemies=[]; fireballs=[]; particles=[]; solids=[]; gates=[]; checkpoints=[]; premiumVisuals=[]; powerups=[]; portalMesh=null;
  }
  ...
  const V52_PRO = {
    label:'V52_RENDER_GAMEPLAY_PREMIUM_PLANEJADO',
    target:'voxel_premium_interativo_no_padrao_referencia',
    active:false,
    visuals:0,
    mechanics:['sword','shield','star','water','pit','lava','shielded_enemy','mini_under']
  };
  ...
  function v52Platform(x,y,z,w,d,top=0x3ed75a,side=0x7a4a22){
    const dirt = box(w,.72,d,side,{outline:true,outlineColor:0x2a1608,outlineOpacity:.10});
    dirt.position.set(x,y,z); v52Add(dirt);
    const grass = box(w*.98,.20,d*.98,top,{outline:true,outlineColor:0xd9ff9b,outlineOpacity:.12});
    grass.position.set(x,y+.46,z); v52Add(grass);
    return {dirt,grass};
  }
  function v52SwordModel(x,y,z){
    const g=new THREE.Group(); g.position.set(x,y,z);
    const blade=box(.16,1.25,.12,0xdbeafe,{outline:true,outlineColor:0xffffff,outlineOpacity:.32,metalness:.25,roughness:.2}); blade.position.y=.48; blade.rotation.z=.6; g.add(blade);
    const h=box(.75,.16,.16,0x38bdf8,{emissive:0x0ea5e9,emissiveIntensity:.22}); h.position.set(.05,-.12,0); h.rotation.z=.6; g.add(h);
    const grip=box(.18,.48,.18,0x5b341a); grip.position.set(-.18,-.46,0); grip.rotation.z=.6; g.add(grip);
    const glow=addGlowSprite(0x38bdf8,1.4,.20); glow.position.y=.05; g.add(glow);
    v52Add(g); g.userData.float={baseY:y,speed:2.1,amp:.18}; g.userData.spin=1.3; return g;
  }
  function v52ShieldModel(x,y,z){
    const g=new THREE.Group(); g.position.set(x,y,z);
    const m=mat(0x22c55e,0x16a34a,{emissiveIntensity:.22,roughness:.32,metalness:.12});
    const s=new THREE.Mesh(new THREE.BoxGeometry(.9,1.0,.16),m); s.castShadow=true; s.receiveShadow=true; g.add(s);
    const core=box(.42,.42,.20,0xbbf7d0,{emissive:0x86efac,emissiveIntensity:.35}); core.position.z=.08; g.add(core);
    v52Add(g); g.userData.float={baseY:y,speed:1.9,amp:.16}; g.userData.spin=.8; return g;
  }
  function v52StarModel(x,y,z){
    const g=new THREE.Group(); g.position.set(x,y,z);
    const m=mat(0xffe259,0xffcc00,{emissiveIntensity:.65,roughness:.22,metalness:.08});
    const core=new THREE.Mesh(new THREE.OctahedronGeometry(.62,0),m); core.castShadow=true; g.add(core);
    for(let i=0;i<6;i++){ const ray=box(.16,.72,.16,0xfff27a,{emissive:0xffcc00,emissiveIntensity:.55}); ray.rotation.z=i*Math.PI/3; ray.position.set(Math.cos(i*Math.PI/3)*.45,Math.sin(i*Math.PI/3)*.45,0); g.add(ray); }
    const glow=addGlowSprite(0xffe259,1.8,.28); g.add(glow);
    v52Add(g); g.userData.float={baseY:y,speed:2.7,amp:.22}; g.userData.spin=2.4; return g;
  }
  function addPowerup(type,x,y,z){
    const mesh = type==='sword'?v52SwordModel(x,y,z):type==='shield'?v52ShieldModel(x,y,z):v52StarModel(x,y,z);
  ...
  function swordAttack(){
    ...
    const blade=box(2.4,.18,.24,0x38bdf8,{emissive:0x38bdf8,emissiveIntensity:.65,transparent:true,opacity:.86});
    ...
    for(const e of enemies){
      if(e.dead) continue;
      const close=Math.abs(e.z-p.z)<5.4 && Math.abs(e.x-p.x)<3.1;
      if(close){
  ...
  function v52EnhanceEnemy(e){
    if(!e || !e.mesh) return;
    if(e.type==='golem' || e.type==='boss'){
      e.shield = e.type==='boss'?2:1;
      const sh=box(e.type==='boss'?1.7:1.15,e.type==='boss'?1.95:1.35,.18,0x38bdf8,{emissive:0x0ea5e9,emissiveIntensity:.30,transparent:true,opacity:.72,outline:true,outlineColor:0xffffff,outlineOpacity:.22});
      sh.position.set(0,0,.72); e.mesh.add(sh); e.shieldMesh=sh;
    }
    if(e.type==='spiky') e.mustUsePower=true;
    if(e.type==='flyer') e.mustCrouchUnder=true;
  }
  function applyV52LevelDesign(world,length){
    if(realBg || !levelGroup || !window.THREE) return;
    document.body.classList.add('v52-profissional');
    const len=length||220; const pal=WORLD[world]||WORLD.field;
    V52_PRO.active=true; V52_PRO.visuals=0;
    // Power-ups e desafios com intenção.
    addPowerup('sword',-2.3,1.15,-26);
    addPowerup('shield',2.4,1.15,-78);
    addPowerup('star',0,1.35,-138);
    // Água atrapalha, poços e lava mais vivos.
    addV52Water(0,-52,4.8,8.5);
    addV52Pit(-3.4,-112,3.5,5.6);
    addV52Lava(world==='fire'?0:4.2,-168,world==='fire'?7.0:3.8,8.0);
    // Ilhas/portal/decoração extra no padrão visual.
    v52Platform(-9,2.2,-42,5.4,4.8,pal.grass2||0x55d75d,pal.dirt||0x7a4a22); v50Tree(-9, -42, pal, true); v50Crystal(-8.2,3.4,-41,pal.magic||0xd13bff,.55);
    v52Platform(8.5,2.0,-72,5.0,4.5,pal.grass2||0x55d75d,pal.dirt||0x7a4a22); v52EnemyStatueReal('slime',8.4,-72,pal);
    v52Platform(-8.8,2.5,-120,5.8,5.2,pal.grass2||0x55d75d,pal.dirt||0x7a4a22); v52EnemyStatueReal('golem',-8.8,-120,pal);
    for(const e of enemies) v52EnhanceEnemy(e);
  }
  function addV52Water(x,z,w,d){
    const m=new THREE.Mesh(new THREE.BoxGeometry(w,.08,d),new THREE.MeshBasicMaterial({color:0x38bdf8,transparent:true,opacity:.46}));
    m.position.set(x,.09,z); v52Add(m); hazards.push({type:'water',mesh:m,x,z,w,d,slow:true});
  }
  function addV52Pit(x,z,w,d){
    const m=box(w,.08,d,0x020617,{outline:true,outlineColor:0x64748b,outlineOpacity:.20});
    m.position.set(x,.08,z); v52Add(m); hazards.push({type:'pit',mesh:m,x,z,w,d});
  }
  function addV52Lava(x,z,w,d){
    const m=box(w,.12,d,0xff3600,{emissive:0xff1800,emissiveIntensity:1.25,roughness:.18,outline:true,outlineColor:0xffd000,outlineOpacity:.25});
    m.position.set(x,.11,z); v52Add(m);
    const glow=addGlowSprite(x,.50,z,0xff4d00,Math.max(w,d)*1.4,.28); hazards.push({type:'lava',mesh:m,glow,x,z,w,d});
    for(let i=0;i<10;i++) { const b=box(.28,.28,.28,0xffc400,{emissive:0xff6b00,emissiveIntensity:.8}); b.position.set(x+(Math.random()-.5)*w,.35,z+(Math.random()-.5)*d); v52Add(b); b.userData.float={baseY:b.position.y,speed:1.5+Math.random(),amp:.18}; }
  }
  function v52EnemyStatueReal(type,x,z,pal){
    const bodyColor=type==='golem'?0x66717d:0x37b24d;
    const g=new THREE.Group(); g.position.set(x,0,z);
    const body=box(type==='golem'?1.7:1.25,type==='golem'?2.25:1.25,type==='golem'?1.25:1.25,bodyColor,{outline:true,outlineColor:0xffffff,outlineOpacity:.20});
    body.position.y=type==='golem'?1.15:.78; g.add(body);
    [-.32,.32].forEach(ex=>{ const eye=box(.22,.16,.09,0xff1744,{emissive:0xff1744,emissiveIntensity:.8}); eye.position.set(ex,type==='golem'?1.62:.98,.66); g.add(eye); });
    if(type==='golem'){ [-.95,.95].forEach(ex=>{ const arm=box(.45,1.25,.45,0x58606a); arm.position.set(ex,.95,0); g.add(arm); }); }
    else { [[0,.48],[-.48,0],[.48,0]].forEach(p=>{ const sp=box(.2,.2,.2,0xd9f99d); sp.position.set(p[0],1.22,p[1]); g.add(sp); }); }
    v52Add(g); return g;
  }
  ...
  function buildLevel(level, worldOverride){
    ...
    applyV52LevelDesign(currentLevel.world, currentLevel.length || 220);
  ...
  function update(dt){
    updateInput(); updateTimer(dt); updatePlayer(dt); updateEnemies(dt); updateFireballs(dt); updateParticles(dt); updatePremiumVisuals(dt); checkCrystals(); checkPowerups(); checkHazards(); checkCheckpoints(); checkGates(); checkPortal(); updateCamera(dt); updateHud();
  }
  ...
  function refreshServiceWorker(){
    if('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js?v=52-profissional-render-gameplay').then(reg => reg.update()).catch(()=>{});
